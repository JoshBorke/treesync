...
local synchedNodes = treeSync.synchedNodes or {}
            ...
            if node then
                ...
                if (parent) then
                    ...
                    if (not importRequestedQue[parentPath]) then
                        ...
                        local serialized = serializer:Serialize("requestNodeData", parentPath)
                        sendRequest(serialized, parentPath, true)
                    end
                end
            end
            telapsed = 0
...
local commandParsers = {
    ...
    ['sendNodeSync'] = function(prefix, rNode, sender, target)
        if target and target ~= __me then
    ...
    ['timeLogin'] = function(prefix, time, sender)
        ...
        AceComm:SendCommMessage(prefix, data, "GUILD", "ALERT")
